...
处置当前 dependency 覆盖的全部事实并提议边界。只读 frozen contextRefs 与唯一 planner DEPENDENCY_RESULT wrapper，不继承会话历史。
...
原型只是目标候选：完整 round/result/Attempt 绑定由 Owner 提供。采用 CODE_ONLY 的 observationKeys 形成独立 Scope Review 意图义务；不生成批准字段，也不为等待流程中必然发生的 fresh Review 增加输入缺口。ScopeDecisionIR 不保存 uncertainty：只有缺少会改变 SOW 高层范围、系统参与方、集成方向、集成方式、目的或责任边界的外部事实时才向宿主报告输入缺口；具体触发时机、endpoint、payload、字段映射、token/claim、重试和错误处理留到迭代开始时确认，不阻断 Scope。
...
DESIGN_ITEM 只能选择 sourceDirectory 中 role=HLD/ADR 且 status=APPROVED 的来源证据。其他来源只能表达需求或原型意图，不能证明批准设计。尚未细化的实现设计不形成 DESIGN_ITEM，也不阻断已经明确的 SOW 高层范围；只有高层范围或责任本身缺失时才请求输入，不能猜节点。
INTEGRATION 的 facetFacts 必须各有一个 DIRECTION/METHOD/PURPOSE；METHOD 只表达 API、文件、消息、OIDC 等高层集成方式，不要求具体协议参数、endpoint 或 payload。NFR 必须有一个 TARGET；其他类型的 facetFacts 为空。字段文本由 Owner 从选中的事实原文生成。
...
每个 factId 只由一条 decision.factIds 处置；其他 root 可引用同一证据或 facetFacts，但不得重复取得事实归属。Integration 的 DIRECTION 应选描述参与系统和交互方向的具体事实，不使用同时列举多项客户提供物的综合责任事实；METHOD 选择来源已经明确的高层集成方式。Integration 的 facetFacts 角色仅为 DIRECTION、METHOD、PURPOSE，不含 TARGET。
