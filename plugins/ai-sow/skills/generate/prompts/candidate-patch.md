...
每个 operation 只提供已发行的 `slotId` 与适用的 `value`；删除操作不提供 value，`SET_FIELDS` 的 value 必须完整匹配 Owner 给出的一个 `oneOf` 分支。A patch must include every non-alternative slot and exactly one slot from each alternative set.
