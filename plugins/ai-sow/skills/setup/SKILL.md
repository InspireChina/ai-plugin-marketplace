...
项目模板来自插件级共享资产 `assets/sow-template.xlsx`，固定包含 `01-需求故事`、`02-任务清单`、
`03-工作量汇总` 和 `90-估算标准`。故事表把 AC 合并为多行文本，任务表直接承载基础单元名称、
工作方式与复杂度；丰富的现状、集成和假设/风险继续留在稳定 JSON 与交付包来源中，不再重复展开到
XLSX。正式模板中的参数表、基础单元目录与公式 prototype 是基础人天、SIT、UAT、取整和总计的
唯一计算依据；Python 和稳定 JSON 不保存计算值，工作簿也不存在风险人天维度。
...
项目模板初始化后是项目级计算输入。当前版本不自动迁移旧项目模板；需要采用新投影时必须由用户
明确授权重新初始化模板。以后只有用户明确授权时，`generate-sow` 才能调整该项目副本；不得反向
修改插件级正式模板资产。
