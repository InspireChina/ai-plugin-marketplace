    ...
    *,
    ...
    if require_no_change:
        ...
        if len(upstream) != 1 or split_ids(upstream[0]) != changed:
            diagnostics.append(diag("REVIEW_UPSTREAM_INVALID", "rebind Upstream must list exactly changed direct owners", review_path))
        ...
        if len(rationales) != 1 or not set(ids).issubset(rationale_ids):
            diagnostics.append(diag("REVIEW_IMPACT_RATIONALE_INVALID", "Impact Rationale must name every stable ID", review_path))
    return diagnostics
