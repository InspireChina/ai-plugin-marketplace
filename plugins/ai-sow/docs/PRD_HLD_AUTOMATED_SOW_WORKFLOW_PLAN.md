...
- 状态：设计已逐节确认，待书面复核和实施计划
- 确认日期：2026-09-02
- 适用范围：`plugins/ai-sow/`
- 方案性质：目标合同，不代表当前插件已经实现
...
输入可以是 Markdown、Word、PDF 或插件明确支持的其他只读格式。标准模板用于降低准备成本，不要求
已有客户文档使用完全相同的标题、目录或版式；校验关注必需语义，不按标题机械拒绝文档。
...
旧实现中仍有效的专业规则、Schema 校验、模板读取和工作簿生成逻辑可以迁移到
...
本文件是后续实施的唯一目标设计。书面方案复核通过后，另行编写文件级实施计划；在实施完成并通过
验证前，当前插件行为仍以 [AI_SOW_PLUGIN_DESIGN.md](AI_SOW_PLUGIN_DESIGN.md) 为准。

未经用户明确要求，本方案落盘不代表开始实施、创建提交、推送或发布版本。
