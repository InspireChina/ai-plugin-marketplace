...
默认资产正式采用 13 列 TaskTable，工作类型名称为可见下拉输入，由公式匹配隐藏的稳定工作类型
ID；原 14 列 ID 输入模板继续兼容。默认任务表删除 SIT 计费点 ID 列，按全表唯一任务名称识别
计费行；“集成类型”仅供目录中具备 `PER_INTEGRATION` 资格的集成任务
选择内部集成/外部集成，其他任务留空。生成前的 Integration 唯一归属校验保留；Excel 人工修改
使用全表任务名称唯一校验，不单独识别两个异名任务是否描述同一方向。费率、复杂度倍率与取整
均保留模板口径。

空白模板预备 60 条故事、200 条任务输入行。`03-工作量汇总` 展示工作量及 8 项参数的受保护
公式视图，原参数表仍位于 `90-估算标准` 隐藏区域。估算标准默认 10 列、右侧 6 列分组细则，
原始机器字段隐藏保留，只冻结前四行。已有项目模板不被默认资产更新覆盖；采用新版须新建 run。

[reference 示例及复现说明](reference/mall订单履约与售后升级_示例说明.md)为 mall 订单履约与
售后升级：22 条 Story、59 条 Task，97.3＋3.5＋3.0＝103.8 人天。公开代码只支持现状事实；
升级需求、企业资产和估算前提均为假设，不是真实历史报价或已批准 SOW。

工作簿固定为 `01-需求故事 / 02-任务清单 / 03-工作量汇总 / 90-估算标准` 四个 Sheet 和五个命名
Table。LibreOffice 真实回算后，固定实现复读公式、缓存、Table 计算列、验证、保护、行高、打印设置、
全部业务行和汇总恒等关系。精简模板的汇总 Sheet 不追加实体 ID、类型或来源引用，保持模板中的工作量与参数布局。追溯信息保留于配套结构化数据和生成证明；原 14 列模板的可见追溯兼容保留。仅转交精简 XLSX 时，不能把缺失的机器 ID 当作已验证的旧身份。
