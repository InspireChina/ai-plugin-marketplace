...
`03-SOW主表` 的验收条件与任务明细使用 `TEXTJOIN + IF` CSE 数组公式汇总，并为每条内容添加项目符号；不得依赖 `_xlfn._xlws.` 动态工作表函数。五张受保护业务表只锁定公式与关系派生单元格及单元格格式；白色输入单元格保持可编辑，同时允许用户调整列宽与行高、使用表头筛选与排序。
