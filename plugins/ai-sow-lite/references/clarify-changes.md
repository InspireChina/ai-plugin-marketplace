...
涉及 AC/备注的专业内容，读取 [目标与验收属于谁](generate-slices.md#目标与验收属于谁)；涉及工作单位、模式或档位，读取 [Task 的实际单位和分类](generate-slices.md#task-的实际单位和分类)；局部未知读取 [未知的有限出口](generate-slices.md#未知的有限出口)。标准的通用成果清单不能自动成为项目 AC；说明/责任/验证前提也须有来源。材料仅说沿用既定规范，就保留该抽象。新局部未知精确保留，不能为消掉问题自行补事实。

用户指定 S/M/L 是实际估算决定，不证明条数等未知事实；仅定档不再调查规模。已有档位和依据未变则保留。首次采用默认 M 可保持分类值，同时更新依据、决定和所解决问题，属于实际修改。若问题还含未答的范围/责任，保留 open，current_handling 写已采用部分和剩余缺口；问题含义/目标实质缩小时提升 revision。resolved 的 resolution 精确含 decision_id/request_id/summary，并有覆盖全部目标的决定；退出义务不是“问题已回答”。
...
按 [观察信封与调用](generate-authoring.md#检查点与活动观察) 使用相同轻量 recorder，必须尽力记录以下实际边界。request_id 属于本次修改；不同执行段沿用请求，可换 execution_id。沿同一大活动复用 activity_id，工具信封附 observation_context。恰好同处的边界与已有工具调用合批，只有语义边界才增加轻量标记；不按对象或思考标记。
