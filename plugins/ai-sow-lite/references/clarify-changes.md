...
| 发现 | 有限处理 |
|---|---|
| 缺事实、方案或责任 | 合批列依据、缺口和影响；全请求共一次定位扩展（定向补读/澄清）。实际不知道、无进展或到限就保存草案与现版 |
...
补充规范按 [引用中的缺口](input-analysis.md#引用中的缺口) 处理：只将真实补足的结果和关键边界纳入有关 AC，保留规范实际名称/版本。一个问题覆盖多个目标而只补足一部分时，更新目标/current_handling 并保留其余缺口；不能因为某个规范或定档获答，就关闭查询定义、迁移映射等其他问题。
