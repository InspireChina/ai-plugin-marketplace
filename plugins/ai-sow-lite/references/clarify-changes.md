...
涉及 Story/AC 修改时复用 [目标与验收属于谁](generate-slices.md#目标与验收属于谁) 的SOW验收定义：简短约定业务结果和关键边界，规范引用注明版本；涉及备注或问题去向时复用 [备注与问题的目标](generate-slices.md#备注与问题的目标)；工作单位、模式或档位依 [Task 的实际单位和分类](generate-slices.md#task-的实际单位和分类)，局部未知依 [未知的有限出口](generate-slices.md#未知的有限出口)。仅在实际影响范围整理；精简AC不要求把原字段、状态码和测试步骤逐项保留在正文，详细约束继续留原材料与引用中，Task 分类、责任及未影响对象保持。不能为消掉问题自行补事实，不另加 AC 评审/回退阶段。
