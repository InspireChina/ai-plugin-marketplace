...
- 此处扩展复杂组合，不把 I1 Excel/保存、I2 输入充分性、I3 确认/竞争的首次验证推迟到这里。
...
- [ ] 创建 EX05/EX07 的六组差异：Story 拆分/合并；公共能力契约与消费者；迁移/上线责任；新历史改变原零匹配；多个问题只答部分/选择独立子集；未拆明工作补齐或明确退出。每组预期分别列必变、必保留、历史出处、未决问题和金额完整性。
...
- [ ] 真实交互优先选择公共能力变化和一组部分子集；其余用完整文件机械夹具验证。实际 agent 若漏影响，修专业分析参考/第一次方案准备；程序越界或错误金额则修对应合同/投影。不要统一靠提高修订上限补救。
...
- [ ] 性能先使用 I2/I3 已采基线。选择小型/典型 generate 和单片 clarify，各做基线与一次目标优化的配对；同输入/模板/模型配置、相同起始上下文和缓存条件。大材料只有发现后段输入/重复回读瓶颈时补一组；结果不稳定才重复对应组，不默认大规模跑分。
