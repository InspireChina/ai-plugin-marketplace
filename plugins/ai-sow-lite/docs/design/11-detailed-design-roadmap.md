...
本文件规划详细设计的层级、主题、依赖和完成标准。[D00 技术栈与运行形态](detailed/D00-technology-stack.md) 已记录可复用基线和来源；[D01/D02/D07/D09](detailed/README.md) 已形成交互与贯穿样例、最小共享合同、保存接口和场景追踪表，[D04](detailed/D04-generate-and-context.md) 已展开骨架、语义分片、主会话与独立子上下文及并发边界。R0—R4 的详细设计正文与 D09 衔接收口已形成，EX01—EX07 提供合成走读与有限探针，I1—I4 给出下一步实现/验收顺序；Lite Skill/运行时、适配和性能仍待实际验证。现有 ai-sow 能力与 Lite 手工模板测试可以减少重复实验，但不等于 Lite 新模型、模板适配、恢复或跨会话修改已完成。
...
这些层级用于组织设计内容，**不对应五层运行时编排，也不是 clarify 的上游回查层数**。回查仍使用 [04](04-bounded-change-and-recovery.md) 中的依据、分析、业务对象和 Excel 责任坐标。
...
现有 `01`—`10` 保留产品规则及概要说明。详细设计集中放在 `docs/design/detailed/`，按专题维护唯一的细节定义；概要只保留原则和链接，避免两处维护字段、状态或接口。[详细设计目录](detailed/README.md) 已包含 D00—D09、D04A/D04B 及 EX01—EX07 样例；D08 的观测合同与有限探针已形成，D09 已完成设计衔接与四个实现增量规划。
...
| D05 Clarify 与修改边界 / L2 | 自然语言定位、讨论与试算、确认有效性、读写集合、语义依赖、回查和有限批次 | Skill 职责草案；方案与确认实例；修改影响表；拆合/清空/问题关闭规则；新会话接续时序 | 指向歧义、部分答复、取消、相关基线变化、越界 diff、回查过深、全局政策变化 |
| D06 Excel 投影与交付 / L3 | 手工模板如何供 agent 使用、业务对象与名称映射、待确认项视图、缺值汇总、扩行、公式与显示 | 逐列写入/只读映射；输出副本布局；计算引擎选择与证据；兼容性和错误处理表 | 空值被 SUM 吞掉、UAT 未知、同名/特殊字符、公式被覆盖、扩行丢保护、内容修复提示、长 AC 裁切 |
...
| Excel 写入与真实重算 | 复用已有投影、扩行、Office 和模板单项证据，验证 Lite 新列映射、问题/缺值展示及公式保护差异 | 优先修适配，保留有效原路径；不为未改能力重做可行性实验 |
...
D03 与 D04 围绕同一样例联合细化材料覆盖、骨架、分片与合并，先通过 [D04A](detailed/D04A-single-session-panorama.md) 看清单 Skill、单 session、无 subagent 的每步输入输出和历史累积，再用实际成本判断读取/输出优化、压缩恢复及是否隔离；并发另评估。D06 以同一份模型验证 Excel 映射、问题展示与缺值传播。不要求专题严格串行编写，也不以子 agent 适配作为基线生成前置。专业内部只写目标、输入、成果、自由度和退出条件，不规定固定阅读顺序或页面 Action。
...
以 [08 场景目录](08-scenario-catalog.md) 为基础，第一轮就指定设计归属与最早检查点，再逐轮补充具体样例。以下是优先贯穿组合，不替代目录中其余案例。
...
以下映射接续 [07 设计缺口](07-gaps-and-decisions.md)，不把缺口重新解释为开工前的用户问卷。
...
**当前已形成 R2 执行/交付与 R3 有限修改设计：** [EX01](detailed/examples/EX01-generate-clarify.md) 覆盖三类范围及局部补值，[D03](detailed/D03-input-analysis-and-exploration.md)/[EX02](detailed/examples/EX02-input-analysis-and-exploration.md) 明确读取/用途/输入问答，D04/D04A 保持单 session 基线，D04B 统一环路，EX03 细化历史候选；[D06](detailed/D06-excel-projection-and-delivery.md)/[EX04](detailed/examples/EX04-excel-projection.md) 已定义逐列映射、问题/续页、分费用缺值、同名与扩行，D02/D07 同步产物接口，D09 指定 163 个场景的主责和最早实现增量。[D05](detailed/D05-clarify-and-change-scope.md)/[EX05](detailed/examples/EX05-clarify-changes.md) 已形成 R3 有限修改设计，覆盖部分答复/确认、拆合与历史引用、试算复用和异常退出，D02/D07 同步最小合同。[D08](detailed/D08-telemetry-and-performance.md)/[EX06](detailed/examples/EX06-telemetry-accounting.md) 已完成观测合同与实验设计，有限只读探针证明当前有 usage 数据载体，尚未证明完整请求和逐活动归属。[D09](detailed/D09-validation-and-implementation.md)/[EX07](detailed/examples/EX07-design-consistency.md) 已补初始化/依据登记、首次空基线、ID 与未拆明工作到公式的接口，四个实现增量已明确；下一步为 I1 可靠程序交付。Excel 和修改差异适配及性能证据随实现补齐。已验证安装和 Excel 基础不重新从零探查，目前没有完成 Lite 运行时或性能实测。
