...
状态：按单人串行、原模板计算的用户决定收口。I1.1 候选校验及 I1.2 文本/分析登记、定向查询、存储与恢复接缝已有实现，当前可调用范围见[工具合同](../../../references/tools.md)，验证与限制见 [I1 记录](../../archive/validation/I1-reliable-delivery.md)。I1.3 已接入真实 Excel 核验及 Generate apply，并通过回归与原生检查，独立审阅及定点复核已通过；D05 的具体修改/确认仍待 I3。本文件包含后续设计合同，不能将所有设计操作视为已经可用。
...
逻辑操作保持以下六项，CLI 统一为 `python scripts/lite.py --request <request.json>`；具体 payload、版本和错误信封见 [P00](../../archive/implementation/P00-contracts-and-fixtures.md)。各操作按实施任务逐项接入，实际支持范围以上述工具合同为准。大参数通过 UTF-8 JSON 请求文件传递，大结果存文件并返回定位；不把整份 PRD/模型反复塞进命令行或 stdout。
...
| `render` | 已检查候选、模板版本、目标候选目录 | 重算并复读的 sow.xlsx、projection.json、工作簿内待确认和完整说明，及检查结果 | 写本请求候选文件；失败保留模型，不能让不完整文件成为成功输出 |
