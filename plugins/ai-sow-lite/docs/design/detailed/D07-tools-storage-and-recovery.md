...
状态：接口与保存协议已接入实现前交接。D05 补齐具体修改/确认，P00 固定 CLI 与编码，P01/P03 安排中断/竞争/取消验证；尚未实现工具，不能把本文件当作已可运行的命令手册。
...
Agent 可在本请求 work 目录写草案；版本目录由工具独占生成和激活，Skill 不通过直接改 current 绕过检查。初版以单写者提交为边界，允许两个请求分别讨论或准备候选，但只能串行生效。没有常驻服务、后台 agent 或默认并行团队。
...
| `check` | 候选包、基线、可选确认方案 | 结构/引用/规则/范围诊断、合法未知、检查内容摘要 | 不代写业务值；失败只指向相关文件/对象/字段 |
| `render` | 已检查候选、模板版本、目标候选目录 | 重算并复读的 sow.xlsx、projection.json、问题视图和必要说明，及检查结果 | 写本请求候选文件；失败保留模型，不能让不完整文件成为成功输出 |
...
`inspect` 的对象选择器同时支持精确 ID、关系端点、问题状态及标题检索。标题模糊命中返回少量候选与所属层级，让 agent 定位；无权自行消歧。读取关系邻域时返回本次查询摘要，供应用前发现新增/删除关系，而不只是校验旧端点没变。
...
[D06](D06-excel-projection-and-delivery.md) 定义 render 的字段映射、可见问题/续页、分类缺值传播和结果合同。正常一次 Office 重算，最终文件复读；不默认额外生成参考工作簿重算。result 只返回产物定位/摘要、引擎身份、分费用完整性与问题数/诊断定位，完整记录存文件。真实重算或保存后再次改变公式/输入，会使已有缓存证据失效；不能把合法未知与计算失败混为一类。
...
| `BASE_STALE` / `SCOPE_EXCEEDED` | Agent 核对差异，必要时重讨论方案 | 相关依据不变可重建候选；实际扩大需新的具体确认 |
...
`projection.json` 是 D02/D06 的 ID/行/名称、字段/问题/续页映射，绑定最终工作簿摘要和投影版本；不增加隐藏 Excel 辅助列。manifest 同时绑定实际 Office 身份、适配规则及最终检查记录。图表、预览、提取缓存和临时 Office 文件不是必须永久保存的业务结果。Excel 内的完整续页属于正式阅读内容，随工作簿保留。

历史匹配的读取集合包含实际 as-is 主题/区域及候选范围的成员/版本摘要；“未匹配”也有读取依赖。新历史材料加入该范围时，代码暴露变化，agent 在有限范围复核 gap/模式；不能只因旧目标来源字节未变就沿用旧零匹配，也不能机械使全项目失效。
...
| 内容 | 精度要求 |
|---|---|
| `read_set` | 本次判断实际依赖的对象/字段摘要、来源版本、模板身份、相关关系查询摘要。模板可先整份核对，后续再按实测需要优化粒度 |
...
确认绑定业务变化、写集合、关键条件和回查边界。措辞或导出路径修复不改变确认语义；换方案、清空原值、改变责任或增加对象则回到具体讨论。技术检查摘要只是防止字节漂移，不形成旧版每阶段 Owner/Reviewer 门禁。

[D05](D05-clarify-and-change-scope.md) 定义确认前形成具体候选、部分选择和试算复用。用户明确只执行已展示且独立闭合的子集时，派生对应有限计划；confirmation 同时保留展示方案定位、所选变化及用户执行意思，绑定实际子集内容摘要，不复用完整方案摘要冒称全部获准。子集需要新的义务/值或改变条件才能成立时，必须先展示修订，不能由工具自行裁剪依赖。确认后复用专业候选；同内容、同模板、同基线的完整有效投影可复用，只有局部试算或内容/基线变化时仍需最终完整投影。
...
文件摘要采用实际字节的 SHA-256；计划/读字段摘要采用版本化的规范 JSON 表达，不规范化业务文本。已确认方案原件保持不变；无关基线更新后的实际应用基线单独记录，不覆盖用户当时看到的基线。规范化细节和跨语言一致样例在实现合同测试中固定，不能把渲染后的 Markdown 当稳定机器摘要。

### 4.1 基线变化

首版可以保守地先比较 current 身份；不同则核对方案读依赖和真实差异：

1. 相关对象、事实、标准或关系邻域变化：旧方案不能直接套用，说明影响并重讨论。
2. 仅无关对象变化，读依赖和方案语义完全不变：复用确认，将已定义的有限变更重建到新 current，保留他人改动；重新检查实际 diff 和新导出，不重做无关专业分析。
3. 无法证明变化无关：保持当前版本，定向补读。不能用自动冲突合并猜业务含义。

这项保守重建是有限变更的重新应用，不是通用 merge 引擎。最终锁内仍要比较期望基线；再次发生竞争返回诊断，不能持锁做分析、调用模型或等待用户。
...
图源：[完整版本生效时序](../diagrams/detailed-save-sequence.mmd)。图中的锁内重复检查覆盖准备期间的竞争；正常重复请求应在准备前快速命中已有结果。

## 6. 幂等、取消与恢复

### 6.1 幂等不等于只看请求名称

请求在实际应用前封存不可变业务意图摘要：generate 绑定交付候选，clarify 绑定已确认的具体方案。正常讨论及允许的方案修订发生在封存前，不能在第一句反馈时锁死未来内容；manifest 保存 request_id、意图摘要和最终应用内容摘要，编码与无关基线重建见 P00。重试使用相同 ID：

- 同请求已成功且意图一致：返回原版本，即使响应丢失；若 current 后来已有更新，也不能把 current 倒回该版本。
- 同 ID 但业务意图不同：拒绝误复用，需新的请求/方案；不按最后一次 payload 覆盖。
- 尚未成功：允许在同一意图内修正序列化、公式写入等问题，内容摘要随候选更新；仍须通过范围、确认与基线检查。
...
| 两个请求竞争 | 先成功者的新版本 | 后者释放锁、核对变化，必要时重建有限候选 |
...
实现前置检查聚焦：源版本迁入独立安装、Lite 候选/模板差异、读写范围、应用生效点、相关/无关基线变化、重复请求及取消信号。详细归属见 [D09](D09-validation-and-implementation.md)。没有要求重放旧插件专业流程来证明这些新协议。
