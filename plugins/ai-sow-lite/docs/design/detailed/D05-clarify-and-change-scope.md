...
Clarify 由同一位用户串行使用，从当前交付和本次反馈开始，尽快给出具体调整方案；用户确认后应用已形成的有限候选，返回新版 Excel 和工作簿内待确认说明。回答一个问题、提出一个建议、要求解释和批准一个已展示方案是不同的意思，不能仅凭收到消息就更新有效 SOW；没有多个改稿请求的并行协调。
...
| 补齐已有规范的适用内容/版本 | 用户要求的相关 Story/AC、实际估算问题及依据/决定 | 仍影响估算的其他目标保留；不因缺开发细则或规范版本继续追问，不顺带改 Task |
...
缺失事实和分类照常进入工作簿内待确认说明，不修改金额、公式或判定估算完整性。相同内容不重复预览，必要修订仍共用 D04B 额度。

候选预览已形成完整文件包，且候选、模板和基线均未变时，确认后可复用；否则按当前候选重新填表。确认记录本身不要求再跑专业分析，预览也不能代替用户对具体修改的确认。

I4.2 的实现采用固定 r1/r2 专业目录与有限 repair/subset 子目录。纯子集保留原完整计划的读取集合、输入快照、条件和边界，每项变化来自实际展示 diff；原计划继续完整复查。选择答复就是执行输入，确认摘要绑定子集与原 shown plan，永久版本归档原展示字节及稳定来源。可变 work/索引只用于应用前核对，避免后续合法输入追加令历史交付失效。机械返修和子集额度详见 [D04B](D04B-bounded-loops.md)，具体字段与例子集中在 [修改参考](../../../references/clarify-changes.md)。
...
这里保留的是影响工作量评估的未决目标，取舍见 [估算相关性](../../../references/input-analysis.md#引用中的缺口)。用户要求清理不影响估算的旧问题时，以具体方案说明工作范围/分类保持，确认后采用撤销追问的 `scope_decision` 并按原合同 resolved；处理说明明确事实未补充，不能编造规范或数量。混合问题只清理不影响估算的部分，旧版本保留。
