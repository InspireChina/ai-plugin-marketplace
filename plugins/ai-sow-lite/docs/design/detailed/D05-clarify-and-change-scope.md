...
Clarify 从当前交付和本次反馈开始，尽快给出具体调整方案；用户确认后应用已形成的有限候选，返回新版 Excel 和剩余待确认项。回答一个问题、提出一个建议、要求解释和批准一个已展示方案是不同的意思，不能仅凭收到消息就更新有效 SOW。
...
复用 D07 的一份计划，不新建审批体系。`changes` 保存明确前后值、拟增删对象及内容引用；`write_set` 保存对象与字段，问题、决定、依据和 lineage 同样列入；`read_set` 与 `read_boundary` 分别表达实际依赖和允许的最深回查。长文本放 work 候选，方案展示变化内容和必要上下文，不展示整个项目 JSON。

用户看到：本次目标、具体前后变化、涉及哪些 Story/Task、公共/交付影响、采用的分类与未决项、最深回查、哪些部分暂缓，以及已知的人天影响或“待模板计算”。工具可在此做一次候选结构/diff 检查；格式错误不应拖到用户确认后才发现。确认后复用该候选，主要执行基线核对、必要检查和交付。
...
默认讨论先展示分类与影响，不为每次聊天重算 Excel。用户确需人天对比，或数值确实决定其选择时，对已明确、有限的候选一次合批用模板试算；不自行造方案、排列组合所有未知或以 Python 另算人天。

试算注明基线、候选和未估算范围；仅局部样本的金额不能充当项目总量差。工作方式/类型确实缺依据时说明不可完整比较，复杂度默认 M 则按当前 M 口径计算并标待确认。语义没有变化不重复试算，同一轮只做一批；修订后的必要计算消耗已有修订/恢复额度。

完整候选试算若已按 D06 生成可交付包，且候选、模板、规则和应用基线均未变，确认后可复用绑定该内容的产物。只有局部试算结果或基线/内容已变，才需要最终完整投影。确认记录本身不要求再跑一次专业分析。
...
| 确认后相关对象、模板或关系邻域变化 | 展示真实差异，在剩余修订额度内重新讨论 | 旧确认不覆盖新语义；无关更新可按 D07 保留确认并重建有限候选 |
...
无关基线变化只允许把明确变更重建到新 current，保留其他人的更新；这不是通用自动 merge。基线变化不能复用旧项目总量/文件缓存，相关读依赖与方案必须重新核对。最终提交锁内仍可能竞争，再次失败按 D04B 退出，不能持续追赶 current。
...
flowchart TD
    ...
    Q -->|是| N{"仅解释或已经采用、无新变化？"}
    N -->|是| R["返回已有结果，请求结束"]
    N -->|否| P["形成有限候选和具体方案<br/>前后值、义务去向、读写与回查边界<br/>复杂度未知 M + 待确认；试算按需合批"]
    P --> U{"用户对具体变化的意思"}
...
优先避免的开销是重复读输入、反复重提同一方案、每句答复都试算、确认后再生成一次候选。全文模型合成和完整公式重算可由工具在磁盘完成，stdout 只返回摘要、定位、diff 和必要诊断。文件落盘不等于主 session 历史已经减少，仍按 D04A 观察实际上下文。

为 D08 提供这些活动边界：接收到首次有效定位/补料反馈、定位与补读、具体方案准备、用户等待、按需试算、确认后检查/导出/应用、失败恢复。记录实际读取区域/对象数、候选写对象/字段数、方案修订、Office 调用、复用与退出原因；用时与 token 只按宿主真实可得粒度归因，不设置新的预算审批。
...
| 试算复用及相关/无关基线变化 | D06 差异实算 + D07 故障注入；不能复用失效金额 | CL05、CL10—CL11、CL28 |
| 保存竞争、取消、响应丢失和中断恢复 | D07 实现故障测试，按实际生效点判定 | FS03—FS08、FS15—FS16、X02—X06 |
