...
片内验收整理统一遵循 [generate-slices](../../../references/generate-slices.md#目标与验收属于谁)：按可测试、无歧义、What 和当前 Story 独立验收组织结果，公共能力的对外契约与业务消费效果分别归属。复杂场景使用条件/触发/结果，简单成果使用短验证点；只有独立成果才拆 Story。来源支持的异常、权限、通知及指标必要条件保持，内部实现约束留原依据及必要工作。此规则用于片内专业生成和一次语义合并，不增设阶段、自动评分或数量门禁。
