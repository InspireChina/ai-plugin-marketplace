...
局部已知工作尚未拆成 Task 时，按 D02 保留对象级 open 问题并设 unestimated_work=true；已有 Task 可算也不能据此宣布该范围估算完整。骨架目标不能因没有生成工作行而丢失。全部本期 gap 已有依据确认为空时，不造占位 Epic/Feature/Story/Task，保留完整分析与排除依据并按 D06 输出“本期无新增工作”；资料不足的空模型不能走这个出口。
