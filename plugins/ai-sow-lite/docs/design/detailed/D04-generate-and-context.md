...
片内验收整理统一遵循 [generate-slices](../../../references/generate-slices.md#目标与验收属于谁) 的SOW定义：用简短条目约定交付后的能力、可验证结果和关键边界，技术/交付Story同样适用。不展开Given/When/Then、完整字段规格或测试步骤；引用缺口按 [估算相关性](../../../references/input-analysis.md#引用中的缺口) 取舍，仅影响工作量的未知绑定实际 Story/AC 或 Task，有据规范可注明实际版本。详细约束留原材料，精简不改变必要工作或关键承诺。公共建设和业务消费各有归属，只有独立成果才拆Story。此规则用于现有片内编写和一次语义合并，不增设阶段、自动评分或数量门禁。
