...
| G11 模板投影与交付核验 | 完整模型、问题/决定、模板、展示映射、输出条件 | 代码读全量数据、填模板、调用计算引擎、复读；agent 处理可定位诊断及必要可见性核对 | 候选 `sow.xlsx`、工作簿内待确认/完整正文、投影及核验结果；局部未知如实说明，不由插件判断金额完整性 | 文件位置、检查摘要/诊断、必要显示样本；Excel 单元格/公式不必整表读回模型 |
