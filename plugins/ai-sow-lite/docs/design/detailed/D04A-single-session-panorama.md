...
| G7 片内联合拆解 | 当前目标、业务/技术/交付依据、验收意图、共享与消费边界 | Agent 按 generate-slices 整理可观察验收成果、公共归属和必要备注，联合形成实际 Task 工作范围 | 本片 Story/AC/Task 候选、引用、跨片关系建议、局部未知；供分类及核对 | 新的专业内容、必要补读和局部判断；不输出全部推理过程作为交付物 |
...
| G11 模板投影与交付核验 | 完整模型、问题/决定、模板、展示映射、输出条件 | 代码读全量数据、填模板、调用计算引擎、复读；agent 处理可定位诊断及必要可见性核对 | 候选 `sow.xlsx`、D 列完整 AC、目标行 open 备注/范围行、投影及核验结果；局部未知如实说明，不由插件判断金额完整性 | 文件位置、检查摘要/诊断、必要显示样本；Excel 单元格/公式不必整表读回模型 |
