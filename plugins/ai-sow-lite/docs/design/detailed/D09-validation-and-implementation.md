...
状态：设计一致性收口与实现规划已形成；D00—D08 的最小合同修订见第 6 节，四个实现增量见第 7 节。已补 EX07 合成反例与场景主责；当前实施进度如下，完整 Skill 使用周期与性能验收尚未完成。
...
- [ ] 按 D05/P00 的既有 check 入口，由 Agent 提交有限编辑、程序构造 work 候选与精确 plan，再形成方案摘要和确认绑定；不新增第七操作，不在此预造字段签名。问题修订、unestimated_work、依据与决定都纳入真实差异，未确认候选不能改变当前版。
- [ ] 确认后只复用同基线的有效候选，应用与导出前检查真实 diff；在 current 异常变化（包括看似无关对象）、已观察取消、越界、Office/文件失败、指针后丢响应的变体中证明保护退出与幂等恢复。BASE_STALE 不自动重建，不复用跨基线确认。
