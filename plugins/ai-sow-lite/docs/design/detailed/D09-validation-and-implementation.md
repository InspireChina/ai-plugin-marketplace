...
本轮按 Q1/Q2 收敛：正常使用为单人串行 generate/clarify，异常基线变化以 `BASE_STALE` 保护退出，不自动重建候选或跨基线复用确认。金额全部由既有模板与 Office 原样计算；只填写白色输入，必要超容量按原公式原型扩行，保留四张表、原公式语义和模板资产哈希。用户最新要求覆盖早先四表输出限制：增加可见的 04-待确认事项、05-完整说明，两表即使为空也保留；实际问题/采用答复及长 AC/正文按原顺序在工作簿内完整可读，主表使用内部链接。Markdown 及 projection path/anchor 继续兼容归档，Excel 阅读不依赖它们；数据合同 lite-projection-v1 不变，渲染实现为 lite-render-v5。专业 Agent 的单 session 基线与未来片内 worker 条件实验保持不变。
...
| 标准与估算权威 | 四项定性分类由 agent 提出，只填写既有白色输入；金额、SIT/UAT 和取整由原模板与 Office 处理。原四表不新增列/汇总/参数/公式体系；输出增加两张文本说明表，不改模板资产哈希 | D06；I1 验证填值、原型扩行及公式/Office 结果保存 |
...
| `runtime/ai_sow_lite/workbook.py`、`runtime/ai_sow_lite/office.py` | D06 的原模板白色输入/名称/原型扩行；I6.1 增加两张可见说明表、逐字续行和内部链接，Markdown 仍归档；既有隔离 Office 调用/复读/超时，保存原公式与 Office 结果，输出同版文件和 projection | I1；I6.1 补独立可读 |
...
这里不要求将所有概念拆成 Python 类。多用途、确认方案、版本清单等结构按 D02/D03/D07 的实际消费者落 Schema/文件；没有必要独立校验的内部值可保留普通数据结构，不为每个名词建立框架。模板原件继续使用现有资产且哈希保持原值；原四表不新增列、汇总、参数或公式体系；输出只增加 D06 约定的两张可见说明表，不新增永久语义实体。
...
|---|---|---|
| 模板保存与合法未知输入 | I1 的白色输入、null/M/未拆明工作、空范围、长原文、原型扩行、原公式/Office 缓存及原生打开证据保留；I6.1 补两张说明表、空态、完整正文/实际答复、内部链接及旧 prepared 拒绝绕过；资产哈希不变 | 仅写入/文件/引用/公式保存失败时修对应机械步骤；Office 原样计算结果不作金额完整性门禁 |
