...
当前实施：I1.1 已完成；I1.2 已实现真实文本/分析登记、有限读取与本地存储/恢复接缝，实际测试、审阅状态和限制见 [I1 验证](../../validation/I1-reliable-delivery.md)。下列整体验收及 I1—I4 复选框仍按增量关闭，不能把候选检查或受控存储包当作完整 I1。I1.3 已接入真实 Excel 投影/核验与公共 Generate apply，回归和原生检查通过，独立审阅及定点复核已通过；I1.4 已实现工具计时与规范化事件报告，相关回归及独立审阅/定点复核通过；I1.5 完整副本验收仍待完成。
...
- [ ] 实现 D02 结构/引用/问题检查，以及 D06 的白色输入映射、必要别名、pending-items.md 和按需 details.md；既有备注/可写单元格引用准确，原文不隐性截断、不覆盖公式列。验证分类齐备、合法缺值、未拆明工作、空范围及超容量扩行；原公式语义、四张表与资产哈希不变，Office 结果原样保存，不增加金额门禁或公式修复。
- [ ] 从第一次可写版本起实现 D07 简单串行保存、首次空基线、幂等和观察到的取消。分别在完整候选准备后、指针切换前后注入中断；保存前基线异常变化须 BASE_STALE 退出，不自动重建或跨基线复用确认。恢复只能返回已生效版本或保留草案，不能依赖事后成功日志。
- [ ] 同步本地工具计时与 D08 最小事件文件/报告；EX06 的重复、计数 epoch、未知与迟到夹具直接验证聚合。无宿主 usage 时仍输出诚实覆盖，事件记录故障不让业务重复执行。
- [ ] 从独立复制的 Lite 目录、另一个普通本地项目目录运行，在 Excel 原生检查原布局、输入、引用、公式及 Office 保存结果。输出同版模型、问题、决定、依据、projection、Excel、pending-items.md、按需 details.md 和版本清单，报告按 D08 独立保存；保留迁入证据、原模板哈希及差异结果。
