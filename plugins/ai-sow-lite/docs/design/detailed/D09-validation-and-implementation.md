...
本轮按 Q1/Q2 收敛：正常使用为单人串行 generate/clarify，异常基线变化以 `BASE_STALE` 保护退出，不自动重建候选或跨基线复用确认。金额全部由既有模板与 Office 原样计算；只填写白色输入，必要超容量按原公式原型扩行，保留四张表、原公式语义和模板资产哈希。待确认事项随 Excel 同版交付为 `pending-items.md`，超长原文按需完整写入 `details.md`，均从既有可写单元格引用。专业 Agent 的单 session 基线与未来片内 worker 条件实验保持不变。

当前实施进度、阶段提交及实测入口统一见 [实施目录](../implementation/README.md)，自主取舍见 [决策记录](../../validation/implementation-decisions.md)。下列 I1—I4 复选框按完整增量的退出条件关闭，不能把设计走读、局部机械检查或受控候选实验当作完整交付。
...
| XL03—XL08、XL10—XL13 | D06 | I1 | 填值/文件/显示 | EX04 未知类型/集成属性、默认 M 待确认、长 AC、公式起始文本；原生实算+机械测试 | 检查白色输入、同版 pending-items.md/details.md 引用及公式保存；仅恢复机械文件步骤 |
...
| XL26 | D06 | I1 | 完整候选/投影 | EX07 已有 Task + 未拆明工作、父对象缺工作行；合同+文件检查 | D02 标记只提示业务范围，在 pending-items.md 可见并由备注引用；不参与公式，关闭/退出需有效方案 |
...
自动 SOW 交付必须证明填值、文件、引用及公式保存正确。模板 SUM 如何处理空白、SIT/UAT 如何计算及最终金额均保留原行为；Office 保存的数值、空白或错误值不构成插件的估算完整性门禁。已有输入漏写、文件损坏或公式被覆盖仍需修复；合法局部未知通过空字段和独立问题说明表达，不能靠改金额处理。业务基础不足由专业分析退出，不进入拆解。
...
| 已有行不等于完整范围 | open 问题的 unestimated_work 只标明尚未形成 Task 的已知业务义务；在 pending-items.md 展示并由既有备注引用，不驱动计算 | D02/D06、XL26；I1 检查文件和引用，I2 检查语义 |
...
| 标准与估算权威 | 四项定性分类由 agent 提出，只填写既有白色输入；金额、SIT/UAT 和取整由原模板与 Office 处理。不新增列/Sheet/汇总/参数/公式体系，不改模板资产哈希 | D06；I1 验证填值、原型扩行及公式/Office 结果保存 |
...
| `runtime/ai_sow_lite/workbook.py`、`runtime/ai_sow_lite/office.py` | D06 的原模板白色输入/名称/原型扩行，独立 pending-items.md/details.md 及其引用；既有隔离 Office 调用/复读/超时，保存原公式与 Office 结果，输出同版文件和 projection | I1 |
...
这里不要求将所有概念拆成 Python 类。多用途、确认方案、版本清单等结构按 D02/D03/D07 的实际消费者落 Schema/文件；没有必要独立校验的内部值可保留普通数据结构，不为每个名词建立框架。模板原件继续使用现有资产且哈希保持原值；输出也不新增列、Sheet、汇总、参数或公式体系。
...
|---|---|---|
| 模板保存与合法未知输入 | I1 白色输入、null/M/未拆明工作与 pending-items.md、空范围、长原文、原型扩行、原公式/Office 最终缓存和原生打开；资产哈希不变 | 仅写入/文件/引用/公式保存失败时修对应机械步骤；Office 原样计算结果不作金额完整性门禁 |
