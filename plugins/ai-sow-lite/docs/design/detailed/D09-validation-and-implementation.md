...
I2.1—I2.3 的最小实现已完成，回归及独立审查通过，见 [I2 验证](../../validation/I2-generate.md)。真实 Skill、历史变体、有限问答、文件续接及原生采集已有证据；原始语义失败、安装反例与局部复验分别保留。旧项目缺历史和不支持的 HLD 格式已补开场批量退出验证。下列跨阶段目标仍保留未验证范围：晚期基础不足、更多宿主的业务执行，以及新增标记的实际覆盖和完整性能对照，在 I4 收口，不把局部通过升级为全部达标。
