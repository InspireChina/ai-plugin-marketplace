...
- [ ] 接续 I1.1 的最小真实候选/受控 usage 探针，在第一个完整真实请求补 D08 起点/终点、原生 scope 与活动标记的集成证据，记录工具/模型时间和 usage 的真实缺口。不得为了凑完整明细另启动模型运行器；G12 有多少证据就报告多少。
...
- [ ] 按 D08/P04 固定小型、典型及至少一组长材料的单 session generate 和局部 clarify 基线，长材料不以小例先发现瓶颈为前提。检查后段义务、决定、问题和回读开销，记录自然压缩是否发生；未发生时沿用 I2 的定向续接演练补证据。保持输入/模板/模型、起始上下文和缓存口径，针对实际开销做少量配对，先改重复读写/回显/候选重写/重算，只在证据支持时评估片内隔离或并发。
...
I4.2的有限返修/子集、真实公共与生产责任变化、独立持续同步方案及复杂历史冲突记录见[I4验证](../../validation/I4-inputs-and-changes.md)。拆合/新历史失效等机械变体与真实专业演练分别标明，不以单例承诺全部语义正确；最终163场景证据与平台限制在I4.3统一列出。
