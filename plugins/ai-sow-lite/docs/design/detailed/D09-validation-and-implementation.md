...
当前实施：I1.1 已完成；I1.2 已实现真实文本/分析登记、有限读取与本地存储/恢复接缝，实际测试、审阅状态和限制见 [I1 验证](../../validation/I1-reliable-delivery.md)。下列整体验收及 I1—I4 复选框仍按增量关闭，不能把候选检查或受控存储包当作完整 I1。I1.3 的真实 Excel 核验接入前，公共 apply 明确拒绝交付。
...
[D06](D06-excel-projection-and-delivery.md)/[EX04](examples/EX04-excel-projection.md) 的模板事实用于核对既有列、公式、Table 和可写范围。Q2 取消输出副本中的金额完整性保护、部分汇总、SIT/UAT 三态及名称 criteria 公式修复；后续只验证白色输入、必要别名/扩行、独立说明引用和 Office 保存的原样结果。当前未生成 EX04 工作簿，也没有正常一次重算的性能实证。
