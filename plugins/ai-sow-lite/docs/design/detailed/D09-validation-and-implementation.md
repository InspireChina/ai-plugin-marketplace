...
当前单人串行 generate/clarify，异常基线变化返回 `BASE_STALE`，不重建候选或跨基线复用确认。D06 按本次反馈采用 v6 原四表原列、open 目标备注/范围行、单格物理限额和唯一旧模板内存兼容；校验原型/A2 说明外的计算口径不变，数据合同仍为 lite-projection-v1，新输出 details=[]、无 details.md。I6.1 与以下既有完成项保留过去实现和实测归属，不证明本次提示效果；专业规则与证据限制见 [AD33](../13-self-review-and-decisions.md#ad33--四表原列与验收成果)。

当前实施进度、阶段提交及实测入口统一见 [实施目录](../implementation/README.md)，自主取舍见 [决策记录](../../validation/implementation-decisions.md)。下列 I1—I4 复选框及第 7 节保留各阶段的原范围和完成证据，不表示 I6.1 已通过。I6.1 的新增验收见 [P01](../implementation/P01-reliable-delivery.md#i61--excel-独立可读增量)，实际验证见 [I6.1](../../validation/I6-self-contained.md)，取舍见 AD32。不能把设计走读、局部机械检查或旧阶段通过当作新范围验证。
...
EX01 不是性能基准本身；它首先是范围和修改语义夹具。正式性能样本需要小型、典型、至少一组固定长材料和公共依赖较多的项目形态，同时保持业务、技术、交付覆盖。长材料基线不以小样本先发现瓶颈为前提；核对后段义务、决定、待确认项和回读开销，记录是否自然压缩，未发生时不声称已验证压缩续接。具体安排见 [P04 I4.3](../implementation/P04-scope-and-performance.md)。
...
在 I1.1 合同首次可校验时，先做 I2 的最小真实候选和受控宿主 usage 探针，尽早暴露专业内容/依据输出负担及计量接入限制；只写 work，不交付用户有效版。随后完成 I1 的可靠程序通道，I2 接通真实 generate 并补集成验证。这样保留正式交付条件，同时不等完整保存、投影和事件聚合都实现后才首次验证 Lite 的关键假设。具体安排见 [P01 I1.1](../implementation/P01-reliable-delivery.md)。
...
I1 实现和独立交付验证已完成，实际证据见 [I1.5](../../validation/I1-delivery.md) 及 [I1 机械能力](../../validation/I1-reliable-delivery.md)。范围与后续加固按自主决策 AD03/AD06/AD09 保留。

### I2 · 真实 generate，不提供预制业务模型

I2.1—I2.3 的最小实现已完成，回归及独立审查通过，见 [I2 验证](../../validation/I2-generate.md)。真实 Skill、历史变体、有限问答、文件续接及原生采集已有证据；原始语义失败、安装反例与局部复验分别保留。旧项目缺历史和不支持的 HLD 格式已补开场批量退出验证。晚期基础撤回后的停止已在[I4.1](../../validation/I4-inputs-and-changes.md)补充，保留旧草稿且无虚假初版。下列跨阶段目标仍保留未验证范围：更多宿主的业务执行，以及新增标记的实际覆盖和完整性能对照，在 I4 收口，不把局部通过升级为全部达标。
...
这里不要求将所有概念拆成 Python 类。多用途、确认方案、版本清单等结构按 D02/D03/D07 的实际消费者落 Schema/文件；没有必要独立校验的内部值可保留普通数据结构，不为每个名词建立框架。模板仅窄改校验原型/A2 说明；唯一旧模板按 D06 在内存兼容，prepared/projection 绑定实际项目 hash。原四表不新增列、汇总、参数或金额公式，不新增永久语义实体。
...
|---|---|---|
| 模板保存与合法未知输入 | I1 的白色输入、null/M/未拆明工作、空范围、长原文、原型扩行、原公式/Office 缓存及原生打开证据保留；I6.1 两张说明表的实测保留历史归属；本次另验原列全文、open 目标备注、范围行、校验优先级、物理超限与唯一旧模板兼容 | 仅写入/文件/引用/公式保存失败时修对应机械步骤；Office 原样计算结果不作金额完整性门禁 |
...
[P00—P04](../implementation/README.md) 将以上增量细化为14项局部实施任务，定义共用机械接口、真实夹具和验证命令；[REVIEW](../REVIEW.md) 汇总设计。实施已按该顺序推进，当前状态以实施目录与对应验证记录为准。专业活动仍由Agent完成；首版能力和性能以实际证据判定，未覆盖的组合继续保留待验证身份。

I4.2的有限返修/子集、真实公共与生产责任变化、独立持续同步方案及复杂历史冲突记录见[I4验证](../../validation/I4-inputs-and-changes.md)。拆合/新历史失效等机械变体与真实专业演练分别标明，不以单例承诺全部语义正确；163场景证据见[清单](../../validation/scenario-coverage.json)，平台与实际性能见[支持矩阵](../../validation/host-support.md)和[性能记录](../../validation/performance.md)。清单含明确机制适用边界，不能据完整编号宣称语义正确率；最终交付检查见[I4交付](../../validation/I4-delivery.md)。
