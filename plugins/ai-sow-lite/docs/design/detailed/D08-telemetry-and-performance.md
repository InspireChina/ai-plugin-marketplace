...
状态：详细设计稿，已做当前 Codex 环境的有限只读探针，未实现 Lite 采集器、执行真实 SOW 性能基准或证明逐活动 token 完整可采。以下字段是 Lite 的拟定合同；宿主字段及实际限制见第 2 节与 EX06，二者不能混用。
...
| 来源 | 已有证据 | 首版使用方式与尚缺证据 |
|---|---|---|
| Lite Python 工具边界 | 可以在本进程开始/结束取得实际时钟；现有 D07 尚无 Lite 实现 | 工具自动记录自身用时、状态、输入输出规模与重试身份；进程外的等待和模型时间另记 |
...
| 当前 Codex 会话记录 | 有限读取确实看到了累计/last usage；该 usage 事件本身未含 turn_id | 仅为候选兼容适配路径；必须按生产者版本验证，并由宿主提供精确会话定位。不能默认扫描聊天目录或把尾部 last 当本活动消耗 |
...
`report.json/md` 是可重建视图，包含 `as_of`、来源事件集合摘要、口径/适配版本、各指标覆盖与诊断。重算以临时文件加替换避免半份报告；迟到事件可在后续真实回调或显式查询时更新报告，但不触发专业重做。
