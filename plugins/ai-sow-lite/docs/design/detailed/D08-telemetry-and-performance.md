...
I7.4 将业务候选是否合法与是否可安全观察分开：被协议 Schema 拒绝或不可调度的请求，只有已有安全 project/request 身份时才记录一次失败工具计时；这两类拒绝请求身份未知或越界时不为遥测创建目录，也不保存无效 payload。首次有效来源登记继续记录工具边界，原业务 validator 和诊断保持权威。外层 Schema 通过的空 payload 同样不能视为可信身份。
