...
Task G 的最终正文同时包含待确认问题/当前处理、非默认分类 rationale 和 notes，布局预算以组合文字为准。完整候选检查在业务与引用检查通过后，复用正式投影正文、模板样式及高度估算，合批报告所有可见高度超限单元格（含派生 H 任务列表）；无需扩展公式、保存工作簿或启动 Office。render 的重新检查和实际投影同样保留整批诊断，按原共享返修额度处理，不改变候选正文或成功工作簿的布局。
