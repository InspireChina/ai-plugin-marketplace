...
引用规则按 [输入分析的统一判断](../../../references/input-analysis.md#引用中的缺口) 处理：材料仅说“已定义条件”“既定映射”或“现有规范”，不能证明具体适用内容和版本已经取得。影响验收的缺口保存在现有主题/输入问题，生成时绑定实际 Story/AC；基础已足够界定时随稿待确认，不为局部缺口增加调查轮次。材料直接给出了足够规则、或只提实现背景时，不因缺文件版本或实现手册制造问题。
