...
XLSX 的 read_id 绑定原工作簿摘要、读取器版本及选项；不能只校验一份可被任意替换的摘录附件。工具先核对原件/附件/读取身份，在检查或重建时按相同规则复核。用户提供的可读副本只定位该副本；不为未支持格式写虚构页码或假 `text_lines`。
...
先将本次相关用户目标/疑点组成有限观察批次，选择代表性路径；输入阅读、原型和历史匹配共用最多一个追加调查批次，详见 [D04B](D04B-bounded-loops.md)。新入口只在能改变重要判断且仍有轮次时纳入追加，不能因不断发现新路径无限探索。
