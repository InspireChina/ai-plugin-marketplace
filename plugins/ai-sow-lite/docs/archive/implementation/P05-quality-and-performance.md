...
**Spec:** [14 · 修复与优化方案](../planning/14-e2e-remediation-and-optimization.md)。状态：I7.1—I7.6 本轮实施与验证完成；性能未达标，最终29项中仍有两项部分通过。各项实证与限制见 [I7 验证](../validation/I7-quality-and-performance.md)，不将已完成实施等同于全部目标达成。
...
新增开发案例目录为 `tests/fixtures/quality/ac-and-decisions/`；其中 `cases.md` 保存来源/反馈，`rubric.md` 保存评审义务，二者分离。复用已冻结的两期资料，不复制一套稍有不同的 PRD/HLD。执行者只收到资料/实际用户反馈与产品指引，评审者读取 rubric。每阶段证据新增到 `docs/archive/validation/I7-quality-and-performance.md`，原报告保留。
...
- [x] 用 B1 错误字段最小请求先复现：返回 `PROTOCOL_INVALID`，但无失败 tool span。
- [x] 新增反例：可信身份+无效 payload 应有一次失败计时；未知/越界身份不写项目；观测失败不改变业务响应；取消保留真实状态；重复采集不加倍。
- [x] 将安全最小身份验证与完整业务校验分离，仅允许前者通过的请求开始观测；原业务 validator 仍返回相同诊断。不要持久化完整无效 payload。
- [x] 修正示例的活动开始位置，跨工具沿同一 activity，等待单列。用真实 Agent 构造/分析耗时跨多个工具的案例核对 start 早于工作、end 晚于成果；不能只包文件保存函数。
- [x] 单一绑定收集全部实际回合并核对独立原生统计；活动 token 无精确映射仍 unknown。故意两个 primary 继续拒绝，不为“通过”删保护规则。
- [x] 定向及完整相关回归通过后记录、提交；建立质量修复后的观察基线，供后两阶段比较。
...
**Files:** 以 `skills/generate/SKILL.md`、`skills/clarify/SKILL.md`、对应编写参考为主要写集合；只有新反例证明现有 helper 不足才改 `authoring.py`。实际串联又定位 AC 底部留白误拒绝，按最小方案追加 `workbook.py`、真实布局夹具/回归、旧 v8 prepared 对照及 D06/tools 同步；不新增接口。评测来源/结果放新 I7 验证记录，旧资料和数字不覆盖。

**Interfaces:** 沿现有 Client 和 `{path,sha256}` 返回引用合批；不增加 `next/submit`、状态机或统一自动执行器。

- [x] 完成一次相同基线、资料、意见和继承模型/effort的有限 M 改稿配对，分方案与交付计量。未触及旧 AC 不冒称全部满足新 rubric；缓存/消费策略与开发环境不完全一致，完整 SOW 同质量隔离基准未建立。记录偏差，不宣称提速。
- [x] 先让 generate 完成简短身份/材料开场，之后再读所需专业参考。Clarify 只读实际对象和准确问题，标准/原文依据能复用即复用；保留真实 read_selectors 与所有必要覆盖，不能靠缩依赖让旧计划过检。
- [x] 确认后直接复用现有 `bind_confirmation → check → render → apply` 连续调用示例。失败即停并保留成功引用；成功响应只输出必要引用和诊断，不再输出完整模型。
- [x] 开展至多两组配对，完整记录每组质量、处理时间、响应数、缓存/未缓存输入、输出、峰值输入、失败成本和未知。缓存状态不同如实说明，不宣称统计显著或保证提速；出现任何义务回退先拒绝该优化。
- [x] 最终运行一次完整 G1→G2→历史Excel→B1→B2→B3；以原29项加Q-AC01—07复核，同一串联内验证AC修改、采用M闭合、部分采纳及重复答复。19项直接通过、6项修正后通过、2项带限制通过、2项部分通过；七项AC义务经首次评审和定点复验通过，首次缺陷保留。
- [x] 新缺陷只定向复验受影响段，旧失败和版本保留。产品完整885通过/12条件跳过、根测试、仓库验证、独立副本/Office与原生视觉检查完成，本项独立提交。未达到的性能、活动计量覆盖与备注精简目标写入限制。
