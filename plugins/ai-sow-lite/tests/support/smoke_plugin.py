...
def install_read_audit():
    ...
    counts = Counter()
    report = dict(read_counts=counts, violations=0, office_conversions=0)
    ...
    def save():
        (workspace / 'audit' / f'{os.getpid()}.json').write_text(json.dumps(report), encoding='utf-8')

    sys.addaudithook(audit)
    atexit.register(save)


def verify_delivery(case, prepared, applied):
    ...
    assert {'model.json', 'pending-items.json', 'decisions.json', 'projection.json', 'sow.xlsx',
            'summary.md', 'pending-items.md', 'details.md', 'verification.json', 'input-records.json'} == files.keys()
    for name in ('model.json', 'pending-items.json', 'decisions.json'):
        assert files[name].read_bytes() == case.file(name).read_bytes()
    ...
    assert manifest['base_version_id'] is None and read_json(files['model.json'])['lineage'] == []
    ...
    assert {ref['path']: ref['sha256'] for ref in manifest['dependencies']} == expected
...
def main():
    ...
    try:
        ...
        process = subprocess.run([str(python), str(plugin / 'tests/support/smoke_plugin.py'), '--worker'],
        ...
        audits = [json.loads(path.read_text(encoding='utf-8')) for path in audit.glob('*.json')]
        counts = Counter()
        for record in audits:
            counts.update(record['read_counts'])
        observed = dict(processes=len(audits), violations=sum(r['violations'] for r in audits),
                        office_conversions=sum(r['office_conversions'] for r in audits), read_counts=dict(counts),
                        scope='Python open audit after startup; Office native reads are not traced')
        assert observed['processes'] >= 8 and observed['violations'] == 0
        assert observed['office_conversions'] == 1  # apply/retry/recover must never recalculate.
