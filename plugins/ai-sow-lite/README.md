...
当前版本：**0.1.0-alpha.3，真实数据问题修复预发布**（[发布记录](https://github.com/InspireChina/ai-plugin-marketplace/releases/tag/ai-sow-lite-v0.1.0-alpha.3)）。已验证 macOS 本地目录中的 Codex 使用流程，以及 Windows 11 上的完整生成与修改链路；Excel 导出需要 LibreOffice（可自行安装，或让插件在自身目录内准备）。完整边界见 [支持与限制](docs/support.md)。
...
历史 XLSX 仅因声明维度或外围空样式膨胀超过读取范围时，可按实际内容边界读取并说明限制，原件保持；真实内容、合并、附注或 Table 超限仍需提供较小的可读副本。
...
Task 备注还会合并待确认文字和必要 notes。完整检查会在导出前一次列出所有预计超过可见高度的行，按诊断合批精简重复表述并保留交付义务；不会自动截断正文。
