...
- 缩短本地与 CI 反馈链：Skill 测试按进程隔离并行，根测试不再重复执行独立 copy-plugin smoke；编排器在单次公开调用内复用已机械验证的不可变 RunEvent 与 ActionLedger 视图，并在事件、Action 或 Attempt 发布后立即失效，减少长流程恢复中的重复全量账本重放，不改变跨调用篡改检测和冻结事实。
