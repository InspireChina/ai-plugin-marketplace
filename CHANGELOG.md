...
- 修复 Schema-invalid 候选含附加属性时 Repair 无法收敛的问题：诊断展开到精确属性路径，Owner 以无值 `REMOVE_FIELD` 槽位只删除 Schema 明确拒绝的现存字段，并与缺失字段修复在同一原子 Patch 中完整复验；预算等待恢复按 Repair 链叶节点优先，不再受 Action ID 排序影响。旧失败 raw 仍保留，新运行使用更新后的 Patch 合同 hash。
