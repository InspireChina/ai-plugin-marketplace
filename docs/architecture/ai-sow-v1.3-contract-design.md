...
状态：已取代的 v1.3 初始设计；不代表当前实现
日期：2026-08-20  
当前插件版本：0.1.0-beta.1
目标 SOW 标准版本：v1.3

> 当前权威合同见 `plugins/ai-sow/docs/AI_SOW_PLUGIN_DESIGN.md` 与 generate Skill。本文保留早期多 Skill、
> 十一 Sheet 和逐层数据所有权方案作为历史记录，现行实现使用单入口与四 Sheet 正式模板。

## 1. 背景与结论

大规模独立测试发现，问题不只是中文输出不自然。当前实现还存在交付层级与工作簿不一致、Skill 职责交叉、过早接入输入、字段含义丢失、需求理由和 Task 内容套用固定模板，以及把开发期工具带入正式流程等问题。

本设计发生于 AI SOW 首次公开发布之前。当前 `0.1.0-beta.1` 直接使用 SOW v1.3；
内部原型未对外发布，因此不提供旧数据迁移器，也不并行读取多版稳定合同。
...
- 不要求 Story 具有固定 Task 类型，但为保持独立验收和结算粒度，每个 Story 最多包含四个 Task。
...
Story 数量和 Task 数量不设置固定比例，但一个 Story 只能归属一个 Feature，且最多包含四个 Task。超过上限表示试拆分已经发现多个可独立验收结果，必须拆分；一个 Story 只有一项 Task 仍可能合理，大规模 Story 都出现同样的一两项通用 Task也属于质量异常。
